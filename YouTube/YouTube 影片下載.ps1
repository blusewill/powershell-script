...
if ($URL.EndsWith(".txt")) {
    if (Test-Path -Path $URL) {
        $todayFolder = Join-Path $CURRENT_PATH (Get-Date -Format "yyyy-MM-dd 影片下載")
        if (-not (Test-Path -Path $todayFolder)) {
            New-Item -Path "$todayFolder" -ItemType Directory
        }
        Start-Process -FilePath "$YT_DLP_PATH" -ArgumentList "--remux-video", "mp4", "--batch-file", "$URL", "--ffmpeg-location", "`"$FFMPEG_PATH`"", "-o", "`"$todayFolder\%(title)s.%(ext)s`"" -Wait -NoNewWindow
    }
}
else {
    Start-Process -FilePath "$YT_DLP_PATH" -ArgumentList "--remux-video", "mp4", "$URL", "--ffmpeg-location", "`"$FFMPEG_PATH`"", "-o", "`"$CURRENT_PATH\%(title)s.%(ext)s`"" -Wait -NoNewWindow
}
